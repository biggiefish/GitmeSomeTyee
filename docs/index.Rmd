...
Normally this is where I would put pictures of all the beauty Tyee I have captured, but that hasn't happened yet. So far, these are the [best things](images/IMG_2815.v2.jpg) I have managed to get in my boat. 
...
## * 3. Escapement Data ----
        ...
        crQ_esc <- read_excel(all.esc.data) %>% 
                          filter(WATERBODY == c("CAMPBELL RIVER","QUINSAM RIVER"),                 # only show data for CR
                                 ANALYSIS_YR >=1990,
...
Ok, well now we know not to bother fishing until early August, that I should book the off the last 3 weeks of August and that adds are that 2023 is not going to set a new record for most Tyee's captured. But who knows. 
...
Well that's interesting. Overall mean fish size is relatively consistent across years. That's good news, but maybe there are better ways to look at this data. Bar plots can be deceptive.

```{r wt boxplot}

wt.box <- ggplot(raw_catch.data) +
...
## ggplot figure
suppressWarnings(ggplotly(wt.box))
```
...
To round this out, let's just have a look at the total number of fish caught per day over the past 20 years. I have included mean daily discharge.
...
```{r, allcatches, echo=FALSE, fig.width = 8, fig.height = 28}
...
We will change gears now and start looking at what may be contributing to observed patterns in catches and size. But first, lets look review what is known about Chinook in the Campbell and in general.
...
## 3.2 What else do we know about current Chinook Salmon trends?
