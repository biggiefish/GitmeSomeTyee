...
2) When fish are most frequently captured.

3) How the size of fish changes within and across seasons. 


##3.1 How catches vary among years.
...
## 3.2 When fish are most frequently captured.
...
## 3.3 Does fish size vary within or between seasons?
...
fish.size <- raw_catch.data %>% mutate(weight = as.numeric(Weight2),
                                ...
                                group_by(Year) %>%
                                mutate(wt.yr.mean = mean(Weight2),
                                          wt.yr.n    = n(),
                                          wt.yr.sd   = sd(Weight2),
...
wt.plot <- ggplot(fish.size, aes(x = Year)) +
                geom_col(aes(x= Year, y = wt.yr.mean, fill = Cycle)) +
...
## ggplot figure
suppressWarnings(print(wt.plot))

 
```

Anyhow, lets see what might be contributing to variability in the number of Tyee captured per year.

Might also be fun to look at daily catches in each year. Given speculation that high flows contributed to record low catches in 2022 lets include some measures of flow.
