...
```{r prep_how many fish should come back, include= FALSE}
## Define assumptions
...
pred.fish <- cr_esc %>% mutate(pred.returns = Esc_Count * perc.female *fecundity *
                                              egg.to.fry * marine.surv,  #fry.to.smolt *
                               Age3_Yr = Year+3,
                               Age3_returns = Age3*pred.returns,
                               Age4_Yr = Year+4,
...
pred.tyee <- pred.fish %>% select(Esc_Count,pred.returns,Age4_Yr, Age5_Yr,Age6_Yr,Age4_tyee,Age5_tyee,Age6_tyee) %>%
                                  pivot_longer(-c(Esc_Count, pred.returns),
                                  ...
                                  summarize(Tyee.Total.Predicted = sum(Tyee.Total),
                                            Esc_Count            = mean(Esc_Count),
                                            pred.returns         = mean(pred.returns)) %>%
                                  mutate(Year = as.numeric(Year))
...
###-------------------------FIX THIS PLOT-------------------------------###
###########################################################################
predicted.plot <- ggplot(pred.tyee, aes(x = Year)) +
                        geom_line(aes(y = Esc_Count, color = Esc_Count))+
                        geom_line(aes(y = pred.returns, color = pred.returns))

print(predicted.plot)
...
total.pred.tyee <- left_join(pred.fish, pred.tyee, by = c("Year")) %>%
                   mutate(Tyee.Prop = Tyee.Total.Predicted/Esc_Count.y,
