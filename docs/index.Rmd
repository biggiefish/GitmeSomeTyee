...
Normally this is where I would put pictures of all the beautiful Tyee I have captured, but that hasn't happened yet. So far, these are the ![best things](images/IMG_2815.v2.jpg) I have managed to get in my boat. 
...
#### 3.1.4 Fecundity
  ...
  +   [Ewart & Anderson, 2013](http://www.crsalmonfoundation.ca/wp-content/uploads/2021/01/CRSF12BYchinreportfinal.pdf) report that female Chinook in the Campbell River in 2012 carried roughly ~5,700, a decrease from the roughly 6,000 eggs typically carried.  
...
#### 3.1.6 Estimated Juvenile Production
  ...
  +   Estimates of juvenile Chinook production based on numbers of observed spawners have generally been less than numbers trapped  throughout the out-migration period ([Thornton et al. 2022](https://www.bchydro.com/content/dam/BCHydro/customer-portal/documents/corporate/environment-sustainability/water-use-planning/vancouver-island/jhtmon-15-Yr-7-2022-07-25.pdf)), which may be due to juvenile survival rates that are relatively higher than typical (e.g. >10%), inaccurate estimates of fecundity, or something else I haven't thought of yet. 
...
```
  ...
  +   However, based on data from [Welch et al. 2020](https://onlinelibrary.wiley.com/doi/epdf/10.1111/faf.12514), survival of coded wire tagged chinook in the Quinsam ranged from `r format(min(surv.quinsam$SAR), digits = 2)` % to `r format(max(surv.quinsam$SAR), digits = 2)` %, with an overall mean of `r format(mean(surv.quinsam$SAR), digits = 2)` %. Which is within the range of other hatchery released sub-yearling populations within the straight of Georgia (see image below, using data  from Welch et al. 2020).  
...
**`r fig_nums("surv.plot", "Survival of coded wire tagged Chinook sub-yearlings (unfed fry) released from hatcheries throughout the Strait of Georgia, including the Quinsam River. Note that y-axis is log transformed to better see range of values (labels are actual values). Figure prepared using data prepared by [Welch et al. 2020](https://onlinelibrary.wiley.com/doi/epdf/10.1111/faf.12514) ")`**
  ...
  Which is lower than average, but comparable to survival reported by other hatcheries within the Strait of Georgia releasing sub-yearling fish and across the west coast of North America (see screenshot of Figure 2, below, from [Welch et al. 2020](https://onlinelibrary.wiley.com/doi/epdf/10.1111/faf.12514))
  
  ![](images/Welch et al. 2020 - Figure 2.jpg)
...
+   Tagging studies have shown that maturing Chinook of BC origin are frequently located in Southeast Alaska, the west coast of Haida Gwaii and west and north coasts of Vancouver Island [Myers et al. 1996](https://npafc.org/wp-content/uploads/2017/08/192USA.pdf) or ![screenshot of map](images/BC.CN.oceanranges.jpg). 

#### 3.1.10 Exploitation Rates

  +   Approximately 20% of fish released by the Quinsam hatchery[Rosenberger et al.2022](https://www.mccpacific.org/wp-content/uploads/2022/01/10Jan2022-SEAK-Catch-of-BC-Salmon-Technical-Report-Part-3-Chinook-Salmon-V1.pdf) are intercepted in Southeast Alaska commercial net (4.99%) and troll fisheries (11.95%), as well as an additional 1.84% that are intercepted in sport fisheries. Interestingly, a [court ruling in May 2023](https://www.documentcloud.org/documents/23795630-jones-order) almost shut down the SE Alaska troll fishery this year. Unfortunately for those aspiring to join the Tyee Club (and Orcas), that decision was [reversed](https://www.documentcloud.org/documents/23855199-panel-decision) in late June, 2023 and the fishery occurred from July 1 to 12, 2023, not sure whether it will resume again at a later date. 
...
fry.flow <- flow_data %>% filter(Year >= 1985) %>%
                          ...
                          filter(life.period == "Incubation")

## Merge fry dataset with max flows dataset.
fry.flow.dat <- fry.prod %>% inner_join(fry.flow, 
                                        by = c('Year' = 'incub.year')) %>%  # merge 'Year' in fry.prod with 'incub.year' in fry.flow data
                              mutate(reduced.fry = ifelse(max.flows >= 350, 0.1*total.fry, total.fry)) %>%
                              filter(Year >= 1991)
 
fry.flow.plot <- ggplot(fry.flow.dat) +
                    geom_col(aes(x = Year, y = total.fry, fill = 'lightblue')) +
                    geom_col(aes(x = Year, y = reduced.fry, fill = "red")) +
                    geom_line(aes(x = Year, y = (max.flows*3750), color = life.period)) +
                    geom_hline(aes(yintercept = 1250000, linetype = 'twodash'), color = "red") +
                    labs(x = "", y = "Number of Fry") +
                    scale_x_continuous(breaks = seq(1991,2021,2)) +
                    scale_y_continuous(labels = comma, limits = c(0,2200000),
                                       sec.axis = sec_axis(~./3750, name = "Discharge (cms)")) +
                    scale_color_manual(values = c("blue", "red")) +
                    scale_fill_manual(values = c("red", "lightblue")) +
                    guides(fill = 'none',
                           color = guide_legend(title = "Max. flow during \nincubation period", 
                                                label = FALSE),
...
[Ewart & Anderson, 2013](http://www.crsalmonfoundation.ca/wp-content/uploads/2021/01/CRSF12BYchinreportfinal.pdf) estimate the marine survival in the Campbell River system is approximately 0.003. This stuff is all way more complicated than I want to get into, so for now I will pretend that marine survival stable (*spoiler, they are not*).

Productivity is another story. There is some thought that periods of high ocean productivity may actually be associated with smaller fish. Some folks have looked at productivity by modelling abundance copepod (which are what fish food eats), but we could also look at something like herring abundance to see if there is any kind of a trend between herring abundance and Campbell River Chinook escapement.  
