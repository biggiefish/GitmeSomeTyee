...
## * 3. Escapement Data ----
        ...
        cr_esc <- esc_data %>% filter(GAZETTED_NAME == "CAMPBELL RIVER",         # only show data for CR
                ...
                ## Prepare dataset of Escapement in CR and Quinsam Rivers 
        crQ_esc <- esc_data %>% filter(GAZETTED_NAME == c("CAMPBELL RIVER","QUINSAM RIVER"),                 # only show data for CR
                                      #ANALYSIS_YR >=2002) %>% # data for same time period as tyee catch data
                                      ANALYSIS_YR >=1990) %>% # data from prior to tyee catch data so we can predict tyee returns.    
                                     mutate(Year          =  as.numeric(ANALYSIS_YR),
                                            Waterbody      =  as.factor(GAZETTED_NAME),
                                     ...
                                     mutate(Esc_Mean       =  as.numeric(mean(MAX_ESTIMATE))) %>%
                               select(Year, Waterbody, Species, esc_count, Esc_Mean)
...
**2.1.9 Ocean Range **
  ...
  +   Tagging studies have shown that maturing Chinook of BC origin are frequently located in [Southeast Alaska, the west coast of Haida Gwaii and west and north coasts of Vancouver Island](images/BC.CN.oceanranges.jpg). 
...
Ok, well now we know not to bother fishing until early August, that I should book the off the last 3 weeks of August and that adds are that 2023 is not going to set a new record for most Tyee's captured. But who knows. Anyhow, lets see what might be contributing to variability in the number of Tyee captured per year.
