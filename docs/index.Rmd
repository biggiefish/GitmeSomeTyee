...
If we look at Escapement trends presented in `r table_nums("plot2", display = "cite")` , we can see:
...
`table_nums("table1", "Size at age of Chinook Salmon captured in Campbell River watershed.")`
...
`r fig_nums("plot5", display = "cite")` shows as strong negative relationship between discharge and the number of fish that are captured. Most likely this is a seasonal effect, where late summer flows are typically low.   
...
Well now we know there is a relationship, lets have a look at some of the older data. 
