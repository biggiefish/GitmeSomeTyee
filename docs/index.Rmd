...
(* more like what do I know about Campbell River Chinook Salmon, which isn't much).
...
```{r predicted.returns.data.prep, include= FALSE}
## Define assumptions
...
pred.age.class.plot <- ggplot(pred.plot.dat, aes(x = Year)) +
                          ...
                          geom_line(aes(y = esc_count, linetype = "Escapement")) +
                          geom_line(aes(y = returning.tyee, color = "Predicted", color = 'dodgerblue')) +
                          geom_line(aes(y = catch_yr.total, color = "Captured", color = 'maroon')) +
                          scale_x_continuous(limits = c(1996,2021)) +
                          labs(y = "# Tyee Salmon", x = "") +
                          guides(linetype = guide_legend(title = "Escapement", label = FALSE),
                                 fill = guide_legend(title = "Predicted Returns \n by Age Class"),
                                 linetype = guide_legend(title = "Number Tyee", label = TRUE)) +
                                 #color = guide_legend(title = "Mean catches per year", label = FALSE)) +
...
`r fig_nums("pred.age.class.plot", "Predicted returns by age-class relative to measured escapement.")`
