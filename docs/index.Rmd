...
cum.catch.plot <- grid.arrange(cum.p1, cum.p2, cum.p3) +
...
## plotly figure
    suppressWarnings(grid.arrange(cum.p1, cum.p2, cum.p3))   
```
**`r fig_nums("grid.arrange(cum.p1, cum.p2, cum.p3)", "Cumulative tally of Tyee's captured by decade.")`**
...
```{r, undersize fish, include = FALSE}
...
## Merge with date_sequence
    ...
    prop.undersize <- left_join(daily.undersize, raw_catch, by = "Date") %>%
                            rename(binary_tyee = catch_binary) %>%
                            mutate(binary_tyee = coalesce(binary_tyee, 0),
                                   cum.tyee.catch    = cumsum(binary_tyee))

    annual.prop.undersize <- prop.undersize %>% 
                             group_by(Year) %>%
                             summarise(total.tyee      = sum(binary_tyee),
                                       total.undersize = sum(undersize),
                                       total.fish      = total.tyee + total.undersize,
                                       prop.tyee       = 100*total.tyee/total.fish,
                                       total.boats     = sum(Boats.AM, Boats.Noon, Boats.PM),
                                       prop.boats      = 100*total.fish/total.boats) %>%
...
knitr::kable(annual.prop.undersize, col.names = c("Year", "# Tyee", "# Undersize Fish", "Total Fish", 
                                                  "% Tyee", "Total Boats", "% Boats Catching"),
...
```
```{r, table2 rower table, echo = FALSE}
kable(rowers.per.decade,
      "html",
      booktabs = T,
      align = c("c"),
      col.names = c("Rower", "2000", "2010", "2020", "Total"),
      caption = "Table 2. Rowers registering the most fish since 2020.") %>%
  kable_styling("striped", full_width =  FALSE,
                position = "left", 
                font_size = 12,
                fixed_thead = TRUE) %>%
  add_header_above(c(" " = 1, "Tyee Rowed" = 4))

```

```{r, table 3 anglers, echo = FALSE}   


kable(Anglers.per.decade,
      "html",
      booktabs = T,
      align = c("c"),
      col.names = c("Angler", "2000", "2010", "2020", "Total"),
      caption = "Table 3. Anglers registering the most fish since 2020.") %>%
  kable_styling("striped", full_width =  FALSE,
                position = "left", 
                font_size = 12,
                fixed_thead = TRUE) %>%
  add_header_above(c(" " = 1, "Tyee Registered" = 4))
```
...
knitr::kable(age_table1, col.names = c("Waterbody", "Age", "n", "% of Total", "Size Range <br>(mm)", "Mean Lenght <br>(mm)"),
             ...
             caption = "Table 2. Size at age of Chinook Salmon captured in Campbell River watershed from [Sturham et al. 1999](https://publications.gc.ca/collections/collection_2014/mpo-dfo/Fs97-4-2477-eng.pdf)")
```
